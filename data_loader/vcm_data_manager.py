...
class VCM(object):
    ...
    def __init__(self,config,min_seq_len=12):
        self.root = config.vcm_data_path

        self.train_root = osp.join(self.root,'Train')
        ...
        #/-----------
        print(os.getcwd())
        discard_train = self._get_names('/data/yyj/discard_train.txt')
        discard_train = [osp.join(self.root, img) for img in discard_train]
        discard_test = self._get_names('/data/yyj/discard_test.txt')
        discard_test = [osp.join(self.root, img) for img in discard_test]
        #/-----------
        ...
        train_ir, num_train_tracklets_ir,num_train_imgs_ir,train_rgb, num_train_tracklets_rgb,num_train_imgs_rgb,num_train_pids,ir_label,rgb_label = \
            self._process_data_train(train_names, track_train, discard_train, relabel=True,min_seq_len=min_seq_len)


        query, num_query_tracklets, num_query_pids, num_query_imgs = \
            self._process_data_test(test_names, track_query, discard_test, relabel=False, min_seq_len=min_seq_len)

        gallery, num_gallery_tracklets, num_gallery_pids, num_gallery_imgs = \
            self._process_data_test(test_names, track_gallery, discard_test, relabel=False, min_seq_len=min_seq_len)


        #--------visible to infrared-----------
        query_1, num_query_tracklets_1, num_query_pids_1, num_query_imgs_1 = \
            self._process_data_test(test_names, track_query_1, discard_test, relabel=False, min_seq_len=min_seq_len)

        gallery_1, num_gallery_tracklets_1, num_gallery_pids_1, num_gallery_imgs_1 = \
            self._process_data_test(test_names, track_gallery_1, discard_test, relabel=False, min_seq_len=min_seq_len)
        #---------------------------------------
    ...
    def _process_data_train(self,names,meta_data,discard_train,relabel=False,min_seq_len=0):
        ...
        for tracklet_idx in range(num_tracklets):
            ...
            if m == 1:
                ...
                img_ir_paths = [osp.join(self.train_root,decoder_pic_path(img_name)) for img_name in img_names]

                #/------------------
                img_ir_paths = [img for img in img_ir_paths if img not in discard_train]
                #/------------------
                ...
                img_names = names[start_index-1:end_index]
                img_rgb_paths = [osp.join(self.train_root,decoder_pic_path(img_name)) for img_name in img_names]

                #/------------------
                img_rgb_paths = [img for img in img_rgb_paths if img not in discard_train]
                #/------------------
    ...
    def _process_data_test(self,names,meta_data,discard_test,relabel=False,min_seq_len=0):
        num_tracklets = meta_data.shape[0]
        pid_list = list(set(meta_data[:,3].tolist()))
        num_pids = len(pid_list)
        ...
        for tracklet_idx in range(num_tracklets):
            ...
            img_names = names[start_index-1:end_index]
            img_paths = [osp.join(self.test_root,decoder_pic_path(img_name)) for img_name in img_names]

            #/------------------
            img_paths = [img for img in img_paths if img not in discard_test]
            #/------------------
