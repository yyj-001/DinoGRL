...
from .graph import k_adjacency, normalize_adjacency_matrix
from .mlp import MLP
...
class MultiScale_GraphConv(nn.Module):
    ...
    def forward(self, x):
        ...
        support = torch.einsum('vu,ncu->ncv', A, x)
        support = support.view(N, C, self.num_scales, V)
        support = support.permute(0,2,1,3).contiguous().view(N, self.num_scales*C, V)
