...
class HorizontalPoolingPyramid():
    ...
    def __call__(self, x):
        ...
        feature_4 = features[2]
        feature_8 = features[3]
        feature_16= features[4]
...
class setpooling(nn.Module):
    ...
    def forward(self, seqs):
        ...
        max = torch.max(seqs, dim=2)[0].unsqueeze(2).repeat(1, 1, t, 1, 1)
        mean = torch.mean(seqs, dim=2).unsqueeze(2).repeat(1, 1, t, 1, 1)
        median = torch.median(seqs,dim=2)[0].unsqueeze(2).repeat(1, 1, t, 1, 1)

        cat = torch.cat((seqs,max,mean,median),dim=1)
        cat = cat.permute(0,2,1,3,4)
        cat = cat.reshape(b*t,4*c,h,w).contiguous()
        cat = self.conv(cat)

        seqs = seqs.reshape(b*t,c,h,w).contiguous()
        seqs = seqs + torch.mul(seqs,cat)
        seqs = seqs.reshape(b,c,t,h,w)
        return torch.max(seqs,dim=2)[0]
...
class Pre_ResNet9(ResNet):
    def __init__(self, channels=[64, 128, 256, 512], in_channel=16, layers=[1, 1, 1, 1], strides=[1, 2, 2, 1], maxpool=False):
